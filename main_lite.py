...
from face_recognizer_lite import FaceRecognizer
import os
...
class CameraFaceRecognition:
    """Real-time face recognition using webcam - Lightweight version"""
    ...
    def __init__(self, csv_path="StudentPicsDataset.csv", confidence_threshold=0.6):
        ...
        """
        print("\n🚀 Initializing Face Recognition System (Lightweight)...")
        ...
        print(f"✅ System ready! Tracking {len(self.recognizer.known_encodings)} students")
    ...
    def run_camera(self, window_width=1200, window_height=800, record_attendance=True):
        ...
        try:
            while True:
                ...
                # Display the frame
                cv2.imshow('Student Face Recognition System (Lightweight)', frame)
    ...
    def _draw_results(self, frame, results):
        ...
        if not results:
            ...
            color = (0, 255, 0) if confidence > 0.85 else (0, 165, 255) if confidence > 0.75 else (0, 0, 255)
    ...
    def _draw_info_panel(self, frame, results, recognized_count):
        ...
        cv2.putText(frame, "STUDENT FACE RECOGNITION SYSTEM (LIGHTWEIGHT)", (20, 40),
        ...
        footer_y = h - 10
        cv2.putText(frame, "Q: Quit | S: Screenshot | R: Reset | Using face-recognition library",
...
if __name__ == "__main__":
    ...
    # Initialize and run
    system = CameraFaceRecognition(csv_path, confidence_threshold=0.75)
